...
describe("LedgerSwitcher (editable-ledger-switch SPEC §5 slice 2)", () => {
  test("renders a nav with a link per available ledger, in canonical order", () => {
    const html = renderToStaticMarkup(
      <LedgerSwitcher
        available={["task-list", "roadmap", "backlog"]}
        active="task-list"
      />,
    );
    ...
    // Canonical order regardless of input order: task-list < roadmap < backlog.
    expect(html.indexOf("Task List")).toBeLessThan(html.indexOf("Roadmap"));
    expect(html.indexOf("Roadmap")).toBeLessThan(html.indexOf("Backlog"));
  });
  ...
  test("renders only the ledgers present in the directory (allow-list)", () => {
    // A directory with just task-list + roadmap — no backlog sibling.
    const html = renderToStaticMarkup(
      <LedgerSwitcher available={["task-list", "roadmap"]} active="task-list" />,
    );
    expect(html).toContain("Task List");
    expect(html).toContain("Roadmap");
